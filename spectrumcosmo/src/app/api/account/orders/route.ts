...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [order] = await sql`
      INSERT INTO orders (
        order_number, user_id, customer_name, customer_email, phone_number,
        total_amount, payment_method, payment_status, notes, status, created_at, updated_at
      ) VALUES (
        ${orderNumber}, ${user.id}, ${customer_name}, ${customer_email || user.email},
        ${phone_number}, ${total_amount || 0}, ${payment_method || null},
        'pending', ${notes || null}, 'pending', NOW(), NOW()
      )
      RETURNING *
    `
    ...
    if (adminEmail) {
      await sendMail({
        ...
        text: `A new order has been placed by ${customer_name}. Order number: ${orderNumber}. Total: $${total_amount}`,
      }).catch(() => null)
    }
