...
export default async function DashboardPage() {
  ...
  try {
    ...
    stats = { products:parseInt(p[0].count), orders:parseInt(o[0].count), reviews:parseInt(r[0].count), pendingReviews:parseInt(pr[0].count) }
    recentOrders = ro
  ...
  return (
    <div className="pt-16 lg:pt-0">
      <div className="mb-8"><h1 className="text-2xl font-bold text-[#111111]">Dashboard</h1><p className="text-gray-500 text-sm mt-1">Welcome back. Here's your store overview.</p></div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {cards.map(({label,value,icon:Icon,href,color}) => (
          <Link key={label} href={href} className="bg-white rounded-2xl p-5 border border-gray-100 hover:border-orange-200 hover:shadow-md transition-all">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${color} mb-3`}><Icon size={18} /></div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <Link href="/admin/orders" className="text-sm text-[#F97316] hover:underline font-medium">View all</Link>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="text-xs text-gray-400 uppercase tracking-wider bg-gray-50">
                ...
                <th className="text-left px-6 py-3">Status</th><th className="text-left px-6 py-3">Date</th>
              </tr></thead>
              <tbody className="divide-y divide-gray-50">
                {recentOrders.map((o:any) => (
                  <tr key={o.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4"><span className={`badge ${STATUS_COLORS[o.status]}`}>{o.status}</span></td>
                    <td className="px-6 py-4 text-xs text-gray-400">{new Date(o.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
