      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        : <div className="overflow-x-auto"><table className="w-full">
            <thead><tr className="text-xs text-gray-400 uppercase tracking-wider bg-gray-50">
              ...
              <th className="text-left px-6 py-3">Details</th><th className="text-left px-6 py-3">Status</th>
                ...
                <tr key={o.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    ...
                    : <div className="relative inline-block">
                        <select value={o.status} onChange={e=>updateStatus(o.id,e.target.value)} className={`appearance-none pr-6 pl-2 py-1 rounded-full text-xs font-medium border-0 cursor-pointer ${STATUS_STYLES[o.status]}`}>
                          {['pending','processing','completed'].map(s=><option key={s} value={s}>{s}</option>)}
                        </select>
