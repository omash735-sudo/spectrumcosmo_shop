...
const navItems = [
  ...
  { name: 'Subscribers', href: '/admin/subscribers', icon: UserPlus, section: 'GROWTH' },
...
function StockAlertBadge() {
  ...
  return (
    <span className="ml-auto bg-[var(--primary)] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center">
      {alertCount > 99 ? '99+' : alertCount}
    </span>
  );
}
    ...
    <>
      ...
      <nav className="flex-1 overflow-y-auto">
        {['CORE', 'DELIVERY', 'SECURITY', 'OPERATIONS', 'GROWTH', 'SYSTEM'].map((section) => {
          ...
          return (
            <div key={section}>
              <p className="text-[9px] sm:text-[10px] font-semibold text-[var(--foreground-muted)] uppercase tracking-wider mt-3 sm:mt-4 mb-1.5 sm:mb-2 px-2">
                {section}
              </p>
    ...
    <div className="min-h-screen bg-[var(--background)]">
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[100] md:hidden">
          ...
          />
          <div className="absolute left-0 top-0 bottom-0 w-[260px] sm:w-[280px] max-w-[85%] bg-[var(--background-card)] shadow-2xl flex flex-col border-r border-[var(--border)]">
            <div className="flex justify-end p-2.5 sm:p-3">
              <button
                onClick={() => setMobileMenuOpen(false)}
                className="p-1.5 sm:p-2 rounded-full hover:bg-[var(--background-secondary)] transition"
              >
