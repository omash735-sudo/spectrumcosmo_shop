      ...
      <main className="min-h-screen bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <nav className="text-sm text-gray-400 mb-8 flex items-center gap-2">
            <a href="/" className="hover:text-[#F97316]">Home</a><span>/</span>
            <a href="/products" className="hover:text-[#F97316]">Products</a><span>/</span>
            <span className="text-[#111111]">{product.name}</span>
          </nav>
          <div className="grid lg:grid-cols-2 gap-16 mb-20">
            <div className="relative h-[500px] rounded-3xl overflow-hidden bg-gray-50">
              <Image src={product.image_url||'https://images.unsplash.com/photo-1576566588028-4147f3842f27?w=800'} alt={product.name} fill className="object-cover" priority />
              <div className="absolute top-4 left-4"><span className="bg-white/90 text-xs font-medium text-gray-600 px-3 py-1.5 rounded-full">{product.category}</span></div>
            </div>
          ...
          {reviews.length>0 && (
            <section>
              <h2 className="text-2xl font-bold text-[#111111] mb-6">Customer Reviews</h2>
              <div className="grid md:grid-cols-2 gap-4">
                {reviews.map((r:any) => (
                  <div key={r.id} className="card p-5">
                    <StarRating rating={r.rating} />
                    <p className="text-gray-700 text-sm mt-3">"{r.review_text}"</p>
                    <p className="text-sm font-medium text-[#111111] mt-3">— {r.customer_name}</p>
                  </div>
                ))}
              </div>
            </section>
