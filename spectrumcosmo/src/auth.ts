...
let authConfig = fallback;

try {
  // Only attempt to initialize NextAuth if required env vars exist
  if (process.env.NEXTAUTH_URL && process.env.NEXTAUTH_SECRET) {
    ...
    console.warn("NEXTAUTH_URL or NEXTAUTH_SECRET missing – using fallback");
  }
} catch (err) {
